...
//		ERR_print_errors_fp(stderr);
		error("error getting public key from request : %s", ERR_error_string(ERR_get_error(),NULL));
...
//		ERR_print_errors_fp(stderr);
		error("error getting subject : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error getting X509v3 extensions : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	/* Only supported extension is subjectAltName */
	subject_altname_pos = X509v3_get_ext_by_NID(req_extensions,	OBJ_sn2nid("subjectAltName"), -1);
...
//		ERR_print_errors_fp(stderr);
		error("error creating X509 certificate : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	/* Set version (X509v3) */
...
//		ERR_print_errors_fp(stderr);
		error("error setting cert version : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error setting serial : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	/* Set subject */
...
//		ERR_print_errors_fp(stderr);
		error("error setting subject : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error setting issuer : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	/* Set public key */
...
//		ERR_print_errors_fp(stderr);
		error("error setting public key : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	/* Set duration */
...
//		ERR_print_errors_fp(stderr);
		error("error setting begin time : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
	if (!(X509_gmtime_adj(X509_getm_notAfter(cert),	SELFSIGNED_EXPIRE_DAYS * 24 * 60))) {
//		fprintf(stderr, "%s: error setting end time", pname);
//		ERR_print_errors_fp(stderr);
		error("error setting end time : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
/* Don't generate v3 extensions in selfsigned.. */
#if 0
...
//		ERR_print_errors_fp(stderr);
		error("error setting subjectAltName : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
#endif
	/* Sign certificate */
...
//		ERR_print_errors_fp(stderr);
		error("error signing certificate : %s", ERR_error_string(ERR_get_error(),NULL));
		exit (SCEP_PKISTATUS_SS);
	}
