...
/**
 * @fn int send_msg(struct http_reply*, int, char*, int, char*, char*, size_t, int, char*, int, char*)
 * @brief
 *
 * @param http
 * @param do_post
 * @param scep_operation
 * @param operation
 * @param M_char
 * @param payload
 * @param payload_len
...
mime_err:
	if (v_flag)
		...
		warning("wrong (or missing) MIME content type");

	return (1);
}

/**
 * @fn void exit_string_overflow(int)
 * @brief
 *
 * @param overflow
 */
void exit_string_overflow(int overflow) {
	if (overflow) {
		//fprintf(stderr, "%s: not enough buffer space to construct HTTP request\n", pname);
		error("not enough buffer space to construct HTTP request");
		exit (SCEP_PKISTATUS_NET);
	}
}

/**
 * @fn char url_encode*(char*, size_t)
 * @brief
 *
 * @param s
 * @param n
...
/**
 * @fn int http_request(struct http_reply*, const char*, const char*, const char*, size_t)
 * @brief
 *
 * @param http
 * @param host_name
                   ...
                   const char *http_string, size_t rlen)
{
	...
	if (rc!=0) {
		...
		error("failed to resolve remote host address %s (err=%d)", host_name, rc);
		return (1);
	}
	...
	if (http_chunked)
	{
		...
		if (rc < 0) {
			...
			error("%i cannot decode chunked payload", rc);
			close(sd);
			return (2);
		}
	}
