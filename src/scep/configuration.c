...
/**
 * @fn int scep_conf_init(char*, int)
 * @brief this function read the configuration file according to the operation
 *
 * @param filename char* : filename of the configuration file
 * @param operation_flag int : operation
 * @return
 */
...
//			ERR_print_errors_fp(stderr);
			error("Error in %s on line %li : %s", filename, err, ERR_error_string(ERR_get_error(),NULL));
	...
	if((var = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION, SCEP_CONFIGURATION_PARAM_MONITORINFO)) && !M_flag) {
		M_flag = 1;
		if(!(M_char_Monitor_Information_HTTP_get_parameter_style = strdup(var)))
			error_memory();
	}
	...
	switch(operation_flag) {
		...
		default:
			...
			error("No operation specified, can't load specific settings!");
			break;
	}
	...
	if(!(engine_section = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION, SCEP_CONFIGURATION_PARAM_ENGINE))) {
		...
		if(!NCONF_get_section(conf, engine_section)) {
			...
			error("Section %s defined but not found!\n", engine_section);
			return(SCEP_PKISTATUS_FILE);
		}
		...
		if((var = NCONF_get_string(conf, engine_section, SCEP_CONFIGURATION_ENGINE_ID))) {
			...
			error("Engine ID not specified, cannot continue. Please provide an engine ID");
			return(SCEP_PKISTATUS_FILE);
		}
		...
		if((cmds_section = NCONF_get_string(conf, engine_section, SCEP_CONFIGURATION_ENGINE_CMDS))) {
			if(!NCONF_get_section(conf, cmds_section)) {
				//fprintf(stderr, "%s: Section %s defined but not found!\n", pname, cmds_section);
				error("Section %s defined but not found!", cmds_section);
				return(SCEP_PKISTATUS_FILE);
			}
...
/**
 * @fn int scep_conf_load_operation_getca(CONF*)
 * @brief read only the configuration needed by the getCA operation in the configuration file
 *
 * @param conf CONF*: pointer on the configuration structure
 * @return 0 if OK
 */
int scep_conf_load_operation_getca(CONF *conf) {
	char *var;
	...
	if((var = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION_GETCA, SCEP_CONFIGURATION_PARAM_FINGERPRINT)) && !F_flag) {
		F_flag = 1;
		if(!(F_char_fingerprint_algorythm = strdup(var)))
			error_memory();
	}
...
/**
 * @fn int scep_conf_load_operation_getnextca(CONF*)
 * @brief
 *
 * @param conf CONF*: pointer on the configuration structure
 * @return
...
int scep_conf_load_operation_getnextca(CONF *conf) {
	...
	if((var = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION_GETNEXTCA, SCEP_CONFIGURATION_PARAM_CERTROOTCHAINFILE)) && !C_flag) {
		C_flag = 1;
		if(!(C_char_CA_certificate_chain = strdup(var)))
			error_memory();
	}
	...
	if((var = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION_GETNEXTCA, SCEP_CONFIGURATION_PARAM_SIGNERCERTIFICATE)) && !w_flag) {
		w_flag = 1;
		if(!(w_char_GetCert_certificate = strdup(var)))
			error_memory();
	}
...
/**
 * @fn int scep_conf_load_operation_enroll(CONF*)
 * @brief
 *
 * @param conf CONF*: pointer on the configuration structure
 * @return
...
/**
 * @fn int scep_conf_load_operation_getcert(CONF*)
 * @brief
 *
 * @param conf CONF*: pointer on the configuration structure
 * @return
...
int scep_conf_load_operation_getcert(CONF *conf) {
	...
	if((var = NCONF_get_string(conf, SCEP_CONFIGURATION_SECTION_GETCERT, SCEP_CONFIGURATION_PARAM_GETCERTSERIAL)) && !s_flag) {
		s_flag = 1;
		if(!(s_char_Certificate_serial_number = strdup(var)))
			error_memory();
	}
...
/**
 * @fn int scep_conf_load_operation_getcrl(CONF*)
 * @brief
 *
 * @param conf CONF*: pointer on the configuration structure
 * @return
...
/**
 * @fn void error_memory()
 * @brief write an error on the stderr stream when there if an error memory
 *
 */
void error_memory() {
	//fprintf(stderr, "%s: memory allocation failure, errno: %d\n", pname, errno);
	error("memory allocation failure, errno: %d", errno);
	//return(1);
}

#if 0
/**
 * @fn void scep_dump_conf()
 * @brief This function dump the configuration on the screen
 *
 */
void scep_dump_conf() {
	...
	int flags[] = {
		c_flag, i_flag, r_flag, v_flag, e_flag, E_flag, F_flag, w_flag, s_flag, l_flag, O_flag, n_flag, T_flag,
		k_flag, K_flag, L_flag, S_flag, p_flag, t_flag, u_flag, v_flag, R_flag
	};
	char *chars[] = {
		c_char_CA_certificate,
		i_char_CA_identifier,
		r_char_GetCrl_CRL_file,
		"true",
		e_char_CA_encryption_certificate,
		E_char_encryption_algorythm,
		F_char_fingerprint_algorythm,
		w_char_GetCert_certificate,
		s_char_Certificate_serial_number,
		...
		L_char_local_self_signed_certificate,
		S_char_Signature_algorithm,
		p_char_proxy,
		"",
		url_char,
		"true",
	...
	t_char = (char *) malloc(sizeof(char) * 20);
	sprintf(T_char, "%d", T_num_MAX_Polling_interval);
	chars[12] = T_char;
	sprintf(n_char, "%d", n_num_Request_count);
	chars[11] = n_char;
	sprintf(t_char, "%d", t_num_Polling_interval);
	chars[18] = t_char;
