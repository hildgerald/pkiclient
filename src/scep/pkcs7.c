...
//				fprintf(stderr, "%s: error writing certificate request in bio\n", pname);
//				ERR_print_errors_fp(stderr);
				error("error writing certificate request in bio : %s",ERR_error_string(ERR_get_error(),NULL));
				return (SCEP_PKISTATUS_P7);
			}else{
				if (v_flag > 1)
		...
		case SCEP_REQUEST_GETCERTINIT:
			...
			if ((rc = i2d_PKCS7_ISSUER_AND_SUBJECT_bio(databio,
						s->ias_getcertinit)) <= 0) {
//				fprintf(stderr, "%s: error writing GetCertInitial data in bio\n", pname);
//				ERR_print_errors_fp(stderr);
				error("error writing GetCertInitial data in bio : %s",ERR_error_string(ERR_get_error(),NULL));
				return (SCEP_PKISTATUS_P7);
			}
			break;
		...
		case SCEP_REQUEST_GETCERT:
			...
			{
//				fprintf(stderr, "%s: error writing GetCert data in bio\n", pname);
//				ERR_print_errors_fp(stderr);
				error("error writing GetCert data in bio : %s",ERR_error_string(ERR_get_error(),NULL));
				return (SCEP_PKISTATUS_P7);
			}
			break;
		...
		case SCEP_REQUEST_GETCRL:
			...
			if ((rc = i2d_PKCS7_ISSUER_AND_SERIAL_bio(databio,
						s->ias_getcrl)) <= 0) {
//				fprintf(stderr, "%s: error writing GetCRL data in bio\n", pname);
//				ERR_print_errors_fp(stderr);
				error("error writing GetCRL data in bio : %s",ERR_error_string(ERR_get_error(),NULL));
				return (SCEP_PKISTATUS_P7);
			}
...
//		fprintf(stderr, "%s: error creating certificate stack\n", pname);
//		ERR_print_errors_fp(stderr);
		error("error creating certificate stack : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	/*
...
//			fprintf(stderr, "%s: error adding recipient encryption certificate\n", pname);
//			ERR_print_errors_fp(stderr);
			error("error adding recipient encryption certificate : %s",ERR_error_string(ERR_get_error(),NULL));
			return (SCEP_PKISTATUS_P7);
		}
...
//		ERR_print_errors_fp(stderr);
		error("error creating data : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("request payload encrypt failed : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag)
...
//		ERR_print_errors_fp(stderr);
		error("error writing encrypted data : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	BIO_flush(memorybio);
...
//		ERR_print_errors_fp(stderr);
		error("failed creating PKCS#7 for signing : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (!PKCS7_set_type(s->request_p7, NID_pkcs7_signed)) {
//		fprintf(stderr, "%s: failed setting PKCS#7 type\n", pname);
//		ERR_print_errors_fp(stderr);
		error("failed setting PKCS#7 type : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error adding PKCS#7 signature : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag > 1)
...
//		ERR_print_errors_fp(stderr);
		error("error adding NID_pkcs9_contentType : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("failed setting PKCS#7 content type : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	...
	if (pkcs7bio == NULL) {
//		fprintf(stderr, "%s: error opening bio for writing PKCS#7 data\n", pname);
//		ERR_print_errors_fp(stderr);
		error("error opening bio for writing PKCS#7 data : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (len != BIO_write(pkcs7bio, buffer, len)) {
//		fprintf(stderr, "%s: error writing PKCS#7 data\n", pname);
//		ERR_print_errors_fp(stderr);
		error("error writing PKCS#7 data : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag)
...
//		ERR_print_errors_fp(stderr);
		error("error finalizing outer PKCS#7 : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag > 1) {
		printf("%s: printing PEM fomatted PKCS#7\n", pname);
...
//		ERR_print_errors_fp(stderr);
    	error("PKCS#7 is not signed! : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
    }
...
//		ERR_print_errors_fp(stderr);
		error("cannot get PKCS#7 data : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("cannot get signer info! : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error verifying signature Error : %s",ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag)
	...
	if(cert_store == NULL)
		...
		error("Can't create a new certificate store");
    ...
    if(!(cert_ctx = X509_STORE_CTX_new()))
    {
    	...
    	error("error creating certificate chain");
    }

    if(!X509_STORE_CTX_init(cert_ctx,cert_store,signercert,NULL))
    {
    	//fprintf(stderr, "%s: error verifying certificates \n", pname);
    	error("error init certificate chain");
    }
    ...
    if( !X509_verify_cert(cert_ctx)) {
    	...
    	error("The signer certificate verification failed");
    }
...
#endif
		{
			//fprintf(stderr, "%s: cannot open cert file for writing\n", w_char_GetCert_certificate);
			error( "%s: cannot open cert file for writing\n", w_char_GetCert_certificate);
...
//			ERR_print_errors_fp(stderr);
			error("error while writing certificate file %s : %s",w_char_GetCert_certificate, ERR_error_string(ERR_get_error(),NULL));
			return (SCEP_PKISTATUS_FILE);
		}else{
			if(v_flag)
...
//		ERR_print_errors_fp(stderr);
		error("error verifying signature : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag)
		printf("%s: signature ok\n", pname);
...
//		ERR_print_errors_fp(stderr);
		error("no attributes found : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
//	if (v_flag)
	...
	if ((i = get_signed_attribute(attribs, nid_messageType,
		...
		error("cannot find messageType");
		return (SCEP_PKISTATUS_P7);
	}
	if (atoi(p) != 3) {
		//fprintf(stderr, "%s: wrong message type in reply\n", pname);
		error("wrong message type in reply");
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag)
	...
	if (( i = get_signed_attribute(attribs, nid_recipientNonce,
		...
		error("cannot find recipientNonce");
		return (SCEP_PKISTATUS_P7);
	}
	...
	for (i = 0; i < 16; i++) {
		if (s->sender_nonce[i] != s->reply_recipient_nonce[i]) {
			if (v_flag)
				//fprintf(stderr, "%s: corrupted nonce received\n", pname);
				error("corrupted nonce received");
			/* Instead of return, break out */
			break;
		}
	}
	/* Get pkiStatus */
	...
	switch (atoi(p)) {
		...
		default:
			...
			error("wrong pkistatus in reply");
			return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("cannot read inner PKCS#7 : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	if (v_flag > 1) {
		printf("%s: printing PEM fomatted PKCS#7\n", pname);
...
//		ERR_print_errors_fp(stderr);
		error("error decrypting inner PKCS#7 : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	BIO_flush(outbio);
...
//		ERR_print_errors_fp(stderr);
		error("error adding data to ASN.1 string : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
...
int get_signed_attribute(STACK_OF(X509_ATTRIBUTE) *attribs, int nid,int type, char **buffer){
	...
	if (*buffer == NULL) {
		...
		error("cannot malloc space for attribute");
		return (SCEP_PKISTATUS_P7);
	}
...
//		ERR_print_errors_fp(stderr);
		error("error creating ASN.1 object : %s", ERR_error_string(ERR_get_error(),NULL));
		return (SCEP_PKISTATUS_P7);
	}
	/* Find attribute */
	for (i = 0; i < sk_X509_ATTRIBUTE_num(attribs); i++) {
		...
		if (OBJ_cmp(X509_ATTRIBUTE_get0_object(x509_attrib), asn1_obj) == 0) {
			if (X509_ATTRIBUTE_count(x509_attrib) != 0) {
				if (*asn1_type != NULL) {
					//fprintf(stderr, "%s: no value found", pname);
